...
// This module helps us make safe calls to external URLs by doing two things:
//
//   * ensuring that no unsafe input can leak into generated URLs
//   * providing convenient validation of response data
//
...
//
//   const url = new SafeUrl('/account/:uid/sessions', 'db.sessions', Joi.array().items(Joi.string()))
//
...
//
//   url.validateResponse([])                 // returns []
//   url.validateResponse(["hello", "world"]) // returns ["hello", "world"]
//   url.validateResponse(42)                 // throws ValidationError
...
module.exports = log => class SafeUrl {
  constructor (path, caller, responseSchema = null) {
  ...
  validateResponse(value) {
    ...
    const {error, validated} = this._responseSchema.validate(value)
    if (error) {
      throw error
    }
    return validated
